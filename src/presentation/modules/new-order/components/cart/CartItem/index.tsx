...
const CartItem = ({product, removeItem}: {product: Product, removeItem: ()=> void}) => {
    return (
      <section className="grid grid-cols-4 mt-10">
          <div className="col-span-2">
              ...
              <div className="flex flex-col ml-2">
                  ...
                  <span className="text-sm text-light-text">Ksh {product.price}</span>
              </div>
              </div>
          </div>
          <div className="text-end">
              <input value={product.quantity} onChange={(e) => product?.setQuantity?.(e.target.value)} type="number" className="w-12 h-12 rounded-lg bg-formBg text-center" />
          </div>
          <div className="text-end">
              <span className="font-medium">KSh {product.totalAmount?.toLocaleString()}</span>
          </div>
          <div className="col-span-4 w-full">
              <div className="flex justify-between mt-3">
                <input type="number" placeholder="Add comment" className="w-full rounded-lg bg-formBg pl-3" />
                <button onClick={()=>removeItem()} className="rounded-lg h-12 border border-accent px-4 ml-2"> 
